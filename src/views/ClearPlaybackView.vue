...
<template>
  ...
  <div class="flex gap-x-6 dark">
    
    <h2 class="text-2xl text-slate-200">
      Clear Playback Positions
    </h2>
    
    <Button btnstyle="outline" btncolor="green" @click="getPlaybackPositions" :disabled="store.loading" class="ml-auto">
  ...
  <div class="pb-48 mt-8 dark">
    
    <div class="mt-6">
      <PlaybackPosnLevel :level="levels" />
    </div>
    
    <div class="fixed bottom-0 left-0 right-0 flex items-center justify-end p-4 bg-gray-900 gap-x-4">
      
      <div>
        {{ selectedEntries.length }} selected:
      </div>
      
      <Button btnstyle="solid" @click="deleteSelectedEntries" :disabled="!selectedEntries.length || store.loading" btncolor="blue">
        Delete entries
      </Button>
